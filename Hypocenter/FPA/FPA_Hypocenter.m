...
%algoritma FPA (Flower Pollination Algorithm)
...
%Membuat Video
v = VideoWriter('Penentuan Hiposenter FA.avi');
open(v);
%Membuat model acak
for ipop = 1 : npop
...
for iitr =  1 : nitr
    for i = 1 : npop
       ...
       model_new = npop_mod;
       tcal_new = t_cal(model_new(1),model_new(2),model_new(3),x,y,z,vp);
    ...
    %Ploting model
hold on
plot3(model_best(1),model_best(2),-model_best(3),'bo','MarkerFaceColor','r','MarkerSize',7)
...
title(['Penentuan Hiposenter || Hasil Inversi ke-.',num2str(iitr)],'FontWeight','bold')
subtitle(['x = ',num2str(model_best(1)),' ; y = ',num2str(model_best(2)),' ; z = ',num2str(-model_best(3)),' || ERMS = ',num2str(Egen(iitr))],'FontWeight','bold')
